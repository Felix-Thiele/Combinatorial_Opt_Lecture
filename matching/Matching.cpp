...
void unshrink(Graph& graph){
    while(graph.has_cycle()){
        ...
        NodeId matched_to_outside_edge = 0;
        for(NodeId i = 0; i < cycle_edges_vec.size(); i++){
         ...
         */
        NodeId size_edges = cycle_edges_vec.size();
        if(debug2) {
            std::cout << matched_to_outside_id << " " << matched_to_outside_edge << "\n";
        }
        for(NodeId j = 0; j < size_edges; j++){
            if(j%2 == 0){
 ...
 * @return
 */
NodeId find_split(std::vector<std::pair<NodeId, NodeId>>& path1, std::vector<std::pair<NodeId, NodeId>>& path2){
...
void delete_tree(Graph& graph){
    ...
    for(NodeId nodeid = 0; nodeid < graph.get_num_nodes(); nodeid++){
        ...
        if(graph.is_in_tree(nodeid)){
            for(NodeId node2id = 0; node2id < graph.get_num_nodes(); node2id++){
                ...
                if(graph.partition().check_combined(nodeid, node2id)){
                    ...
                    if(node2.is_matched() and node2.get_match_neighbors()[0] < node2id){
                        graph.add_final_matching_edge(std::make_pair(node2.get_match_neighbors()[0], node2id) );
