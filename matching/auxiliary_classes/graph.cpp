...
bool Graph::is_matched(NodeId n){
    for(NodeId m = 0; m < get_num_nodes(); m++){
...
void Graph::add_cycle(std::vector<std::pair<NodeId, NodeId>>& path1,
    ...
    for(int i = last_common_index; i < static_cast<int>(path1.size()); i++){
    ...
    for(int i = static_cast<int>(path2.size())-1; i >= last_common_index; i--){
...
void Graph::remove_tree_edge (NodeId node1_id, NodeId node2_id)
{
  ...
  for(NodeId i = 0; i < get_num_nodes(); i++){
